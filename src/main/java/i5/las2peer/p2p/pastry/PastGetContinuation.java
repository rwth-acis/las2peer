 ...
 * 
 * For usage, you can either do something else while waiting for the result of the past(ry) retrieval and check the
...
public class PastGetContinuation<T> implements Continuation<PastContent, Exception> {
	...
	/**
	 * Simple reference to the nested class. This is necessary for checking the class of a result received from the
	 * pastry backend.
	 * 
	 */
	...
	/**
	 * this method will be called by the past(ry) backend, if an exception occurred during the attempted retrieval of
	...
	/**
	 * this method will be called by the past(ry) backend, if the artifact has been feteched succesfully
	 * 
			...
			try {
				if (content instanceof ContentEnvelope) {
					...
					if (!genericClass.isInstance(this.content)) {
						ex = new PastryStorageException(
								"Content type '" + content.getClass().getName() + "' does not fit expected class!",
					...
					ex = new PastryStorageException(
							"expected a ContentEnvelope but got a " + content.getClass().getName());
				}
