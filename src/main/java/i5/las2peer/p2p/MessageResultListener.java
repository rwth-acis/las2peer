...
/**
 * A MessageResultListener is a simple collector for one result of a message sending operation
 * in a {@link Node}. 
 * The listener will be called by the node, when the result of the message or any exception 
 * in between has arrived.
 * Since a message may be delivered as multi- or broadcast, multiple results (and exceptions)
 * are possible.
 * 
 * The original sending part may do something else while waiting for the message result or use
 * the {@link #waitForAllAnswers()} or {@link #waitForOneAnswer} methods.
 * 
...
public class MessageResultListener {
	 ...
	 */
	public Exception[] getExceptions () { return exceptions.toArray( new Exception[0]); }
	
	/**
	 * @return	array with all collected results
	 */
	public Message[] getResults () { return answers.toArray(new Message[0]); }
	 ...
	 */
	public boolean isTimedOut () { return status == Status.TIMEOUT; }
	 ...
	 */
	public boolean hasException () { return exceptions.size() > 0; }
	
	/**
	 * @return	number of successfully collected results
	 */
	public int getNumberOfResults() { return answers.size(); }
	
	/**
	 * @return	the number of expected results
	 */
	public int getNumberOfExpectedResults () { return this.numberOfRecipients; }
	...
	/**
	 * check if this method is times out, call the corresponding notification hook,
	 * if the timeout is detected here
	 *   
	 * @return	true, the the message is times out
	 */
	public boolean checkTimeOut () {
		synchronized ( this ) {
			if ( inWaitMethod || isFinished()) return false;
	 ...
	 * 
	 * This hook may be overwritten at listener usage or generation e.g. for decoupled
	 * implementation of message success observation.
	 * 
	...
	/**
	 * notifyTimeout will be called, if the underlying network has decided, that
	 * the corresponding message has timed out and passed a corresponding exception
	 * to this listener.
	 * 
	 * This hook may be overwritten at listener usage or generation e.g. for decoupled
	 * implementation of message success observation.
