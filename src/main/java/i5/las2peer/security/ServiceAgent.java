...
public class ServiceAgent extends PassphraseAgent {
	 ...
	 */
	protected ServiceAgent ( long id, String serviceClass, KeyPair pair, String passphrase, byte[] salt ) throws L2pSecurityException, CryptoException{
	...
	public String toXmlString() {
		try {
			return 
					"<las2peer:agent type=\"service\" serviceclass=\""+getServiceClassName()+ "\">\n"
					+"\t<id>" + getId() + "</id>\n"
	...
	public void receiveMessage(Message m, Context c ) throws MessageException {
		try {	
			...
			if ( content instanceof RMITask ) {
				getRunningAtNode().observerNotice(Event.SERVICE_INVOCATION, m.getSendingNodeId(), m.getSender(), getRunningAtNode().getNodeId(), this, this.getServiceClassName() + "/" + ((RMITask) content).getMethodName());
				...
				try {
					...
					if ( thread.hasException() ) {
						...
						if ( thread.getException() instanceof InvocationTargetException 
							...
							getRunningAtNode().observerNotice(Event.SERVICE_INVOCATION_FAILED, m.getSendingNodeId(), m.getSender(), getRunningAtNode().getNodeId(), this, "Exception: " + thread.getException() );						
						}
					} else {
						response = new Message ( m, new RMIResultContent ( thread.getResult() ));
						getRunningAtNode().observerNotice(Event.SERVICE_INVOCATION_FINISHED, m.getSendingNodeId(), m.getSender(), getRunningAtNode().getNodeId(), this, this.getServiceClassName() + "/" + ((RMITask) content).getMethodName());
					}
					response.setSendingNodeId( getRunningAtNode().getNodeId());
				} catch (InterruptedException e) {
					response = new Message (m, new RMIExceptionContent ( e ));
					getRunningAtNode().observerNotice(Event.SERVICE_INVOCATION_FAILED, m.getSendingNodeId(), m.getSender(), getRunningAtNode().getNodeId(), this, "Exception: " + e );
				...
				throw new L2pServiceException ( "I don't know what to do with a message content of type " + content.getClass().getCanonicalName());
			...
			throw new MessageException ("security problems - " + m.getRecipient().getId() + " at node " + getRunningAtNode().getNodeId(), e);
	 ...
	 * @throws L2pSecurityException 
	 */
	public static ServiceAgent generateNewAgent ( String forService, String passPhrase ) throws CryptoException, L2pSecurityException {
	 ...
	 */
	public Object invoke(String method, Object[] parameters) throws IllegalArgumentException, NoSuchServiceMethodException, IllegalAccessException, InvocationTargetException, L2pSecurityException {
		if ( ! isRunning () )
	 ...
	 * @throws L2pSecurityException
	 */
	public Serializable handle ( RMITask task ) throws L2pServiceException, ServiceInvocationException, SecurityException, IllegalArgumentException, NoSuchServiceMethodException, IllegalAccessException, InvocationTargetException, L2pSecurityException {
		if ( !getServiceClassName().equals(task.getServiceName()))
			throw new L2pServiceException ( "Service is not matching requestes class!" + getServiceClassName() + "/" + task.getServiceName()  );
