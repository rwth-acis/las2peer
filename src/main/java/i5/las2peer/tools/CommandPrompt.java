...
public class CommandPrompt {
	...
	private boolean methodMatches(Method method, String methodName,
		...
		if (methodParameterTypes.length != types.length) return false;
	...
	public ReturnStatus handlePrint ( String line ) {
		...
		for ( int i=1; i<split.length; i++ ) {
			if ( htLocals.containsKey(split[i])) {
				ColoredOutput.println ( "   " + split[i] + ": " + htLocals.get(split[i]) + " (" + htLocals.get(split[i]).getClass().getCanonicalName() + ")");
			} else 
