...
/**
 * This class can be used to test LAS2peer functionalities. It is an extended version of the
 * {@link L2pNodeLauncher} class that contains additional methods for server testing.
 * It should only be used for LAS2peer core development and is not meant for productive usage,
 * since some of these methods are not extensively tested and actively supported.
 *  
 * @author Peter de Lange
 *
 */
public class ExtendedTestLauncher {
	 ...
	 */
	private ExtendedTestLauncher ( int port, String bootstrap, boolean monitoringObserver, L2pClassLoader cl ) {
	 ...
	 * @param cl the classloader to be used with this node
	 */
	private ExtendedTestLauncher ( int port, String bootstrap, int nodeNumber, boolean monitoringObserver, L2pClassLoader cl ) {
	 ...
	 * @throws NodeException
	 */
	static ExtendedTestLauncher launchSingle ( String[] args, int nodeNumber, File logDir, L2pClassLoader cl) throws NodeException {
		int port = Integer.parseInt(args[0].trim());
		String bootstrap = args[1];
		ExtendedTestLauncher launcher;
		...
		if (args.length > 3 && args[2].equals("startObserver")){
			launcher = new ExtendedTestLauncher (port, bootstrap, nodeNumber, true, cl);
			startWith++;
		}
		else{
			launcher = new ExtendedTestLauncher (port, bootstrap, nodeNumber, false, cl);
		}
		try {
			if ( logDir != null )
	...
	private static L2pClassLoader setupClassLoader(String[] serviceDirectory) {
		return new L2pClassLoader( 
			new FileSystemRepository (serviceDirectory), ExtendedTestLauncher.class.getClassLoader() 
		);
	}
...
/*		System.out.println ("\nStart Multiple Nodes: (Note: currently not fully working)");
		...
		for ( Method m : ExtendedTestLauncher.class.getMethods()) {
	...
	public static void main ( String[] argv ) throws InterruptedException, MalformedXMLException, IOException, L2pSecurityException, EncodingFailedException, SerializationException, NodeException  {
		...
		if ( argv[0].equals ("-s")) {
			...
			ExtendedTestLauncher launcher = launchSingle( args, -1, logfileDirectory, classloader);
