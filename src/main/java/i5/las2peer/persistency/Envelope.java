...
public final class Envelope implements XmlAble, Cloneable {
	...
	/**
	 * may the content of this envelope be overwritten by {@link #updateContent} or just via the object
	 * returned by {@link #getContent} 
	 * 
	...
	/** 
	 * base constructor for generating instances to be filled via {@link fromXmlString}
	 * used by the factory {@link createFromXml}
	 */
	private Envelope () {
		bOpen = false;
	}
	...
	/**
	 * create a new envelope for the given content readable by the 
	 * given owners
	 * 
	 * a new envelope will always be closed at the end of the creation process
	 * 
	 ...
	 * @throws DecodingFailedException 
	 */
	public Envelope(String content, Agent owner) throws UnsupportedEncodingException, EncodingFailedException, DecodingFailedException {
		this ( content, new Agent[] { owner });
	}
	 ...
	 * @throws EncodingFailedException
	 */
	private Envelope ( String content, Agent[] owners, long id ) throws UnsupportedEncodingException, EncodingFailedException {		
		this.id = id;
		bOpen = true;
	 ...
	 * @throws UnsupportedEncodingException 
	 */
	private Envelope ( XmlAble content, Agent[] owners, long id ) throws UnsupportedEncodingException, EncodingFailedException {
		this.id = id;
		this.bOpen = true;
		initKey();
		initOwners(owners);
	 ...
	 * @throws SerializationException
	 */
	private Envelope (Serializable content, Agent[] owners, long id ) throws EncodingFailedException, SerializationException {
		this.id = id;
		this.bOpen = true;
	...
	/**
	 * close the envelope, i.e. encrypt all data with the key
	 * and forget the key
	 * 
	 * @throws EncodingFailedException
	 */
	...
	private void decryptData () throws DecodingFailedException {
		if ( baPlainData != null ) return; // nothing to do
	...
	/**
	 * try to return the contents of the envelope as a real object which has been serialized 
	 * via XmlAble
	 * 
	...
	public String toXmlString () {
		...
		return
			"<las2peer:envelope lastchange=\""+ timestamp +"\""
	...
	public void setStateFromXml(String xml) throws MalformedXMLException {
		...
		try {
			...
			if ( ! keys.getAttribute("encoding").equals( "base64"))
				throw new MalformedXMLException("base 64 encoding of the content expected - got: " + keys.getAttribute("encoding"));
			if ( ! keys.getAttribute("encryption").equals(CryptoTools.getAsymmetricAlgorithm()))
				throw new MalformedXMLException(CryptoTools.getAsymmetricAlgorithm() + " encryption of the content expected");
	 ...
	 * // possible: extend identifier with counter, if a collision occurs  
	 * 
	 * @param cls
	 * @param identifier
	 * 
	 ...
	 * @throws UnsupportedEncodingException
	 */
	public static Envelope createClassIdEnvelope ( Object content, String identifier, Agent[] owners ) throws EncodingFailedException, SerializationException, UnsupportedEncodingException {
	 ...
	 * @throws UnsupportedEncodingException 
	 */
	public static Envelope createClassIdEnvelope ( Object content, String identifier, Agent owner ) throws UnsupportedEncodingException, EncodingFailedException, SerializationException {
	...
	/**
	 * check, if the current envelope may be overwritten by the given one.
	 * If the current envelope is not signed, overwriting is permitted.
	 * 
	 * Otherwise, the new envelope must contain at least one signature of the signing agents
	 * if this envelope.
	 * 
	...
	public void checkOverwrite(Envelope envelope) throws L2pSecurityException  {
		...
		throw new L2pSecurityException ( "Check for Overwriting envelope " + getId() + " failed: No needed signature is provided!");
	}
